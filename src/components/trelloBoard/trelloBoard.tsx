...
const TrelloBoard = () => {
    ...
    const configureTrello = async (document: Document, firstTime?: boolean) => {
        ...
        await loadScripts(
            document,
            `https://trello.com/1/client.js?key=${trelloKey}`
        )
    ...
    return (
        <>
            <div
                className={`w-full h-full pt-8 ${styles.container} ${utilStyles.flexCol_N}`}
            >
                <div className={`${utilStyles.flexRow_Centre}`}>
                    ...
                    <h2 className={`text-lg text-fl-blue font-medium`}>
                        {GlobalState.name}
                    </h2>
                </div>
                ...
                <div className={`my-8 ${utilStyles.flexRow_N}`}>
                    <div className={`mx-4 ${utilStyles.flexCol_Centre}`}>
                        ...
                        <div
                            className={`${styles.scrollWrap} ${utilStyles.flexCol_N}`}
                        >
                    ...
                    <div className={`mx-4 ${utilStyles.flexCol_Centre}`}>
                        <h2
                            className={`text-lg mb-4 text-fl-green font-medium`}
                        >
                            Done
                        </h2>
                        ...
                        >
                            {GlobalState.doneCards.length === 0 ? (
                                <div
                                    className={`${styles.bgdImageWrap} ${styles.bgdDone}`}
                                >
                                    <MarkDone />
                                </div>
                            ) : (
            ...
            {showTaskModal && (
                <Modal
                    modalCloses={{ closeModal: () => setShowTaskModal(false) }}
                    children={<Task />}
                />
            )}
                    ...
                    children={
                        <div className={`${utilStyles.flexCol_Centre}`}>
                            <h1 className={`text-3xl text-white text-center`}>
                                Hi! To see your Trello board, <br /> please
